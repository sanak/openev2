...
class FunctionDefsParser(TypesParser):
        ...
        def define_func(self, name, retType, args):
                ...
                for arg in args:
                        ...
                        if argType == 'string':
                                ...
                                varDefs.add('PyProgressData', 'sProgressInfo')
                                parseStr = parseStr + 'O'
                                ...
                                if nullok:
                                        ...
                                        extraCode.append(nullokTmpl %
                                                         (argName, argName, objects[argType],
                                                          argName, objects[argType][0:index] +
                                                          "_IS" +
                                                          objects[argType][index:],
                                                          argName,
                                                          argName, argType))
                                        argList.append(argName)
                                elif default:
        ...
        def decodeRet(self, impl, funcCall, retType, nullret):
                if retType == 'none':
                    ...
                    if nullret:
                        impl.write('    retval = (GObject *)')
                        impl.write(funcCall)
                        impl.write(';\n    if (retval) return PyGtk_New(retval);\n    Py_INCREF(Py_None);\n    return Py_None;\n')
                    else:
                        impl.write('    return PyGtk_New((GObject *)')
                        impl.write(funcCall)
                        impl.write(');\n')
                else:
